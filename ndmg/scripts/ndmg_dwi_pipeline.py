...
def ndmg_dwi_worker(dwi, bvals, bvecs, t1w, atlas, mask, labels, outdir,
	...
	try:
	    ...
	    print('Aligned Atlas: ' + label)
            ...
            cmd='tck2connectome ' + namer.dirs['output']['fiber'] + '/tractogram.tck ' + parc_mif + ' ' + namer.dirs['output']['conn'].values()[0] + '/connectome.csv -tck_weights_in ' + namer.dirs['output']['fiber'] + '/weights.csv -out_assignments ' + namer.dirs['output']['conn'].values()[0] + '/assignments.csv -symmetric -assignment_radial_search 4 --force'
	    print(cmd)
            os.system(cmd)
            cmd='tck2connectome ' + namer.dirs['output']['fiber'] + '/tractogram.tck ' + parc_mif + ' ' + namer.dirs['output']['conn'].values()[0] + '/meanlength.csv -tck_weights_in ' + namer.dirs['output']['fiber'] + '/weights.csv -scale_length -stat_edge mean -symmetric -assignment_radial_search 4 --force'
            os.system(cmd)
            ...
            os.system(cmd)
	    g1 = mgg.graph_tools(namer=namer, connectome_path=namer.dirs['output']['conn'].values()[0] + '/connectome.csv')
	    g1.make_graph()
